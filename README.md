...
The data entry can be done either automatically by providing a .csv file containing pairs of floating point data seperated by a comma or manually if no command line argument (CLA) is provided. 
...
When run, the program presents the user options for printing to the console the folowing items:
- The raw data
...
Besides that, if data was entered manually, the program provies the user with the option to export the manual data to a .csv file.

## Usage
As mentioned in the description, there are two methods for data entry with this program: csv and manual imput. 
As the program flow is based initially on this choice, each method will be explained seperately. 
...
If the user types 1 and then <enter>, the raw data from data.csv is printed to the console.
...
Options 0, 2, and 3 function identically as they do when entering data via CSV. 
